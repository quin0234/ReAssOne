...
function missPicture(item){
    let playerPic = item.picture;
    //console.log(item.picture);
    if (playerPic == ""){
        console.log('no picture')
        playerPic = "http://images.clipartpanda.com/soccer-player-silhouette-9TperMa8c.jpeg";
    }
}
    ...
    render: function() {
        return (
            React.createElement("ul", {className: "nav-menu"},
                React.createElement("li", {className: "link1"},
                    React.createElement("a", {href: "#"}, "Squad List")
                ),
                React.createElement("li", {className: "link2"},
                    React.createElement("a", {href: "#newitem"}, "Add Player")
                )
            )
        )
    }
...
function setState(changes) {
    ...
    switch(splittedUrl[0]) {
        case "newitem":
            ...
            break;
        case "item":
            let playerList = state.items
            component = ItemPage;
            componentProperties = playerList.find(i => i.key == splittedUrl[1]);
            console.log(items);
            break;
        default:
            component = MainPage;
...
setState({location: location.hash, 
    playerItem:{
        ...
        position: "",
        number: "",
        picture: "http://images.clipartpanda.com/soccer-player-silhouette-9TperMa8c.jpeg"
    },
    items: [
